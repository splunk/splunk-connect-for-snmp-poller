...
from splunk_connect_for_snmp_poller.manager.task_utilities import get_handler, walk_handler, mib_string_handler, parse_port, build_authData, build_contextData
    ...
    try:
                    ...
                    for varbind in varBinds:
                        ...
                        if isinstance(varbind, list):
                            ...
                            try:
                                ...
                                if len(varbind) == 3:
                                    mib_index = varbind[2]
                                mib_string_handler(snmp_engine, auth_data, context_data, host, port, varbind[0], varbind[1], mib_index, mib_server_url, server_config, results)
                            ...
                            try:
                                if varbind[-1] == "*":
                                    walk_handler(snmp_engine, auth_data, context_data, host, port, varbind, mib_server_url, results)
                                else:
                                    get_handler(snmp_engine, auth_data, context_data, host, port, varbind, mib_server_url, results) 
                            except Exception as e:
        ...
        # Perform SNNP Polling for oid profile in inventory.csv
            ...
            if profile[-1] == "*":
                ...
                walk_handler(snmp_engine, auth_data, context_data, host, port, profile, mib_server_url, results)
            # Perform SNNP GET for an oid 
            else:       
                logger.info(f'Executing SNMP GET for {host} profile={profile}')
                get_handler(snmp_engine, auth_data, context_data, host, port, profile, mib_server_url, results) 
