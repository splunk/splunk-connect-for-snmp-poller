...
from splunk_connect_for_snmp_poller.manager.poller_utilities import (
    ...
    return_database_id, InventoryRecord,
)
...
class Poller:
            ...
            for ir in parse_inventory_file(self._args.inventory):
                ...
                if entry_key in inventory_hosts:
                    logger.error(
                        ...
                        "the same device", ir.__repr__(), ir.host, ir.profile)
                    continue
    ...
    def is_conf_changed(self, entry_key, ir, old_conf):
        ...
        return old_conf != (ir.host, ir.version, ir.community, ir.profile, config, indexes) or frequency != interval
...
def scheduled_task(ir: InventoryRecord, server_config, splunk_indexes):
    logger.debug(f"Executing scheduled_task for %s", ir.__repr__())

    snmp_polling.delay(ir, server_config, splunk_indexes)
